...
import func_rob
import math
from func_pickplace import pickplace,espera
	...
	if newvalor>valor:
		...
		if operacion=="COORDENADAS":
			...
			try:
				...
				cursor.execute(sql)
				conn.commit()
		elif operacion=="MOVER A UN PUNTO":
			try:
				sql= f"""
				    UPDATE tblur3
				    SET ESTADO='ACTIVO'
				...
				conn.commit()
				rob=func_rob.createrob()
				func_rob.movepunto(rob,int(x),int(y),int(z))
				x,y,z,rx,ry,rz=rob.getl()
				sql= f"""
				    UPDATE tblur3
				    SET X='{x}', Y='{y}', Z='{z}', RX='{rx}', RY='{ry}', RZ='{rz}', ESTADO='INACTIVO' 
				...
				cursor.execute(sql)
				conn.commit()
		elif operacion=="ACTIVAR GRIPPER":
			try:
				sql= f"""
				    UPDATE tblur3
				    SET ESTADO='ACTIVO'
			...
			try:
				...
				cursor.execute(sql)
				conn.commit()
		elif operacion=="AGREGAR USUARIO":
			try:
				newuser=df.iloc[-1]['usuarios']
				newpin=df.iloc[-1]['pin']
				sql= f"""
				    INSERT INTO tblusuarios (pin, usuario)
				    VALUES ('{newpin}','{newuser}');
			    """
				cursor.execute(sql)
				conn.commit()
				...
				sql= f"""
				    UPDATE tblur3
				    SET ESTADO='USUARIO AGREGADO CORRECTAMENTE' 
				    WHERE id = '{newvalor}';
			    """
				cursor.execute(sql)
				conn.commit()
